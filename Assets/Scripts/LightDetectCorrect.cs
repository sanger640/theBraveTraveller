...
public class LightDetectCorrect : MonoBehaviour {
	...
	void Update () {
		...
		if (object1.GetComponent<LightController>().getState() == answer1 && 
			object2.GetComponent<LightController>().getState() == answer2)
			state = true;
		else state = false;
